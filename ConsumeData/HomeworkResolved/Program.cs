...
namespace HomeworkResolved
{
    class Program
    {
        ...
        public static async Task InsertRow(string name, int age)
        {
        ...
        public static async Task UpdateRows(int id, string name, int age)
        {
            string connectionString = ConfigurationManager.ConnectionStrings["MyConnection"].ConnectionString;
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand("UPDATE Clients SET Name = @name, Age = @age WHERE id = @id", connection);
                await connection.OpenAsync();
                command.Parameters.AddWithValue("@id", id);
                command.Parameters.AddWithValue("@name", name);
                command.Parameters.AddWithValue("@age", age);
                int numberOfUpdatedRows = await command.ExecuteNonQueryAsync();
                ShowGreenMessage($"Se actualizó {numberOfUpdatedRows} ciente(s).");
            }
        }
        public static async Task DeleteRows(int id)
        {
            string connectionString = ConfigurationManager.ConnectionStrings["MyConnection"].ConnectionString;
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand("Delete Clients WHERE id = @id", connection);
                await connection.OpenAsync();
                command.Parameters.AddWithValue("@id", id);
                int numberOfUpdatedRows = await command.ExecuteNonQueryAsync();
                ShowGreenMessage($"Se eliminó {numberOfUpdatedRows} ciente(s).");
            }
        }
    }
}
